...
class LogController extends Controller
{
		...
		if($student == NULL){
			...
			if($approved == 0){
				...
				if($books_issued >= $max_allowed){
					return 'Siswa tidak dapat menerbitkan buku lagi';
				} else {
					...
					if($book == NULL){
						...
						return 'ID Edisi Buku Tidak Valid';
						...
						if($book_availability != 1){
							return 'Buku tidak tersedia untuk diterbitkan';
						} else {
	...
	public function edit($id)
	{
		...
		if(!$log->count()){
			return 'ID Buku yang dimasukkan tidak valid atau buku sudah dikembalikan';
		} else {
    ...
    public function renderIssueReturn(Request $request) {
