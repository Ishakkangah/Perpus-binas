...
class AccountController extends Controller
{
	...
	public function postSignIn(Request $request) {
		...
		if($auth) {
			...
			return Redirect::route('account-sign-in')
				->with('global', 'Email salah or Password salah.');
		}

		return Redirect::route('account-sign-in')
			->with('global', 'Ada masalah. Sudahkah Anda mengaktifkan akun Anda?');
	}
	...
	public function postCreate(Request $request) {
		...
		if(!$validator) {
			...
			if($userdata) {			
				...
				return Redirect::route('account-sign-in')
					->with('global', 'Akun anda telah dibuat. Kami telah mengirimi Anda email untuk mengaktifkan akun Anda');				
			}
		}
	}
